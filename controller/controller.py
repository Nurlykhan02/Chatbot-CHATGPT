...
from utils import send_message, create_openai_thread, send_to_leadvertex, openai_thread_send_message
    ...
    async def webhook_connect(self, request: Request, db: Session = Depends(get_db)):
        try:
            ...
            if is_new_lead and is_new_customer and message_type == 'message.incoming':
                ...
                user_info = CustomersTable(saID = said, phone = client_phone, openai_thread = thread_id)
                db.add(user_info)
                db.commit()
